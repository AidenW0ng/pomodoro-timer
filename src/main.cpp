...
int main() {
  ...
  int user_state = 0;
  sf::SoundBuffer alarm_end;
  sf::Sound alarm_end_sound;
  alarm_end.loadFromFile("src/sounds/end_alarm.mp3");
  alarm_end_sound.setBuffer(alarm_end);

  sf::ContextSettings settings;
  settings.antialiasingLevel = 8;
  bool clock_running = false;
  sf::VideoMode desktop = sf::VideoMode::getDesktopMode();
  sf::RenderWindow window(sf::VideoMode(500, 500, desktop.bitsPerPixel),
                          "Timer", sf::Style::Default);
  sf::Clock clock;
  ...
  time_display.setPosition(window.getSize().x / 2.0f,
  ...
  std::vector<sf::Text> overhead_button_text = {text_study, text_short_break,
                                                text_long_break};
  std::vector<sf::RectangleShape> overhead_buttons = {
  ...
  text_stop.setPosition(
      stop_button.getPosition().x + stop_button.getSize().x / 2,
  ...
  for (int i = -1; i <= 1; i++) {
    ...
    sf::FloatRect overhead_text_bound =
        overhead_button_text[i + 1].getLocalBounds();
    text_short_break.setCharacterSize(20);
    sf::FloatRect ycoordadjust = text_short_break.getLocalBounds();
    sf::FloatRect overhead_button_bound =
        overhead_buttons[i + 1].getLocalBounds();
    ...
    overhead_buttons[i + 1].setOrigin(
        overhead_button_bound.left + overhead_button_bound.width / 2.0,
        overhead_button_bound.top + overhead_button_bound.height / 2.0);
    overhead_buttons[i + 1].setPosition(
        time_display.getPosition().x + (i * (time_text_bound.width / 2.0 - 30)),
        time_display.getPosition().y - 80);
    overhead_button_text[i + 1].setFillColor(sf::Color::Black);
    overhead_button_text[i + 1].setOrigin(
        overhead_text_bound.left + overhead_text_bound.width / 2.0,
        ycoordadjust.top + ycoordadjust.height / 2.0);
    overhead_button_text[i + 1].setPosition(
        overhead_buttons[i + 1].getPosition().x,
  ...
  while (window.isOpen()) {
    ...
    while (window.pollEvent(event)) {
      if (event.type == sf::Event::Closed) {
        ...
        time_display.setPosition(event.size.width / 2.0f,
                                 event.size.height / 4.0f);
      }
    }
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (bound.contains(static_cast<sf::Vector2f>(mouse_pos)))
        overhead_buttons[i].setFillColor(sf::Color(186, 31, 46));
      else
